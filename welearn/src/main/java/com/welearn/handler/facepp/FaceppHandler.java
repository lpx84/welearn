...
public class FaceppHandler {
	...
	public JSONObject deletePerson(String studentNo) {
		...
		try {
			...
			// System.out.println("\nperson/delete");
			System.out.println();
			JSONObject json = httpRequests.personDelete(new PostParameters()
					.setPersonName("person_" + studentNo + "_0"));
			return json;
		} catch (FaceppParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	/**
	 * 查询两张图片的相似度调用此方法，这里的path取从数据库读取的值，因为域名在方法内加了,值在80以上基本上就可以判断 是同一个人
	 * 
	 * @param avatarPath
	 *            学生头像的path
	 * @param picPath
	 ...
	 */
	public Double getSimilarity(String avatarPath, String picPath)
			throws JSONException, FaceppParseException {

		JSONObject avatarRes = httpRequests
				.detectionDetect(new PostParameters().setUrl(rootUrl
						+ avatarPath));
		if (avatarRes.getJSONArray("face").length() == 0) {
			throw new FaceppParseException("avatarPath：没有发现人脸！");
		}

		String avatarFace = avatarRes.getJSONArray("face").getJSONObject(0)
				.getString("face_id");

		JSONObject picRes = httpRequests.detectionDetect(new PostParameters()
				.setUrl(rootUrl + picPath));
		if (picRes.getJSONArray("face").length() == 0) {
			throw new FaceppParseException("picPath：没有发现人脸！");
		}

		JSONArray picFaces = picRes.getJSONArray("face");
		Double res = 0.0;
		for (int i = 0; i < picFaces.length(); ++i) {
			String faceid = picFaces.getJSONObject(i).getString("face_id");
			// 对比两张人脸的相似程度
			JSONObject compare = httpRequests
					.recognitionCompare(new PostParameters().setFaceId1(
							avatarFace).setFaceId2(faceid));
			if (compare == null) {
				throw new FaceppParseException("没有获取到相似度数据！");
			} else {
				Double similarity = compare.getDouble("similarity");
				res = (similarity>res?similarity:res);
			}
